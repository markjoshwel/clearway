"""List unread Teams direct chats (1:1 and group chats).

This script discovers the Teams database location and displays all
unread direct messages, excluding meetings and channel conversations.
"""
...
# Ensure we use the local libteamsdb
sys.path.insert(0, str(Path(__file__).parent / "libteamsdb"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="List unread Teams direct chats",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                    # List all unread chats
  ...
  %(prog)s --list-sources     # Show discovered database locations
    ...
    parser.add_argument(
        "--exclude-hidden",
        action="store_true",
        help="Exclude hidden/archived conversations",
    )
...
def list_unread_chats(
    ...
    """List unread chats from the Teams database.

    Args:
        db_path: Specific database path, or None to auto-discover
        exclude_hidden: Whether to exclude hidden conversations
    ...
    # Extract and display unread chats
    try:
        ...
        for conv in conversations:
            # Filter for chat type only
            if conv.thread_type != ThreadType.CHAT:
                continue

            # Exclude meetings
            if "meeting" in conv.id.lower():
                continue

            # Exclude system/internal conversations (48: prefix is Teams internal)
            if conv.id.startswith("48:"):
                continue
            ...
            if conv.unread_count > 0:
                unread_found = True
                unread_chats.append(conv)

        if not unread_found:
            print("No unread direct chats found.")
            return 0

        print(f"Unread Direct Chats ({len(unread_chats)} conversation(s)):\n")

        for conv in unread_chats:
